...
public class BzmBuild implements Callable<Result, Exception>, Serializable {
        ...
        try {
            ...
            try {
                ...
                if (master != null) {
                    ...
                    if(builder.getWebhookUrl() != null && !builder.getWebhookUrl().isEmpty()) {
                        sendWebhookUrl(master.getId(), build.getCurrentTest().getName(), build.getPublicReport(), builder.getWebhookUrl());
                    }
                    if(builder.getWebhookUrlSlack() != null && !builder.getWebhookUrlSlack().isEmpty()) {
                        sendWebhookUrlSlack(master.getId(), build.getCurrentTest().getName(), build.getPublicReport(), builder.getWebhookUrlSlack());
                    }
                } else {
    ...
    public void sendWebhookUrl(String masterId,String testName,String publicReportUrl,String webhookurl){
        PrintStream logger = listener.getLogger();
        BuildResult buildResult = build.doPostProcess(master);
        String color = "";
        ...
        try {
            ...
            logger.println(BzmJobNotifier.formatMessage("Response Code for Teams: " + responseCode));

        }
        catch (Exception e){
            e.printStackTrace();
            logger.println(BzmJobNotifier.formatMessage("Error generated in send webhookurl teams"));
        }
    }
    public void sendWebhookUrlSlack(String masterId,String testName,String publicReportUrl,String webhookurl){
        PrintStream logger = listener.getLogger();
        BuildResult buildResult = build.doPostProcess(master);
        String color = "";
        ...
        try {
            ...
            int responseCode = connection.getResponseCode();
            logger.println(BzmJobNotifier.formatMessage("Response Code for Slack: " + responseCode));
            ...
            e.printStackTrace();
            logger.println(BzmJobNotifier.formatMessage("Error generated in send webhookurl For Slack"));
        }
    }
